...
public class StraightFlushEvaluator extends AbstractEvaluator
{
  ...
  public StraightFlushEvaluator(int ranking, int handSize, int maxCardRank)
  {
    ...
    // perform other initialization
	  super(ranking, handSize, "Straight Flush", handSize, maxCardRank);
  }
  @Override
	public boolean canSatisfy(Card[] mainCards) {
		
		for (int i = 0; i < mainCards.length-1; i++) {
			if (mainCards.length >= cardsRequired() && mainCards[i].getRank()-1 == mainCards[i + 1].getRank()
					&& mainCards[i].getSuit() == mainCards[i+1].getSuit()) {
				i++;
  ...
  /* public boolean canSatisfy(Card[] mainCards) {
	for(int i = 0; i < mainCards.length-1; i++) {
	if( mainCards.length >= cardsRequired() &&mainCards[i].getSuit() == mainCards[i+1].getSuit() && mainCards[i].getRank() == mainCards[i+1].getRank()+1) {		
	return true;
}
